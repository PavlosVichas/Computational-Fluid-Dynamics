%%%%%%%%%%%%% CFD 2ND ASSIGMENT IMPLICIT SCHEME VICHAS PAVLOS AEM 6663 %%%%%%%%%%%%%%%
clear
clc
close all

%%% - (2*U/StepY) for left + (2*U/StepY) for right
%%%%% TO CHANGE REYNOLDS PLEASE MODIFY KINEMATIC VISCOCITY (Re =1 --> V = 1)
%%%%% (Re =1   --> V = 1)
%%%%% (Re =100 --> V = 0.01)
%%%%% (Re =500 --> V = 0.002)

%problem_case = 'ΟΜΟΡΟΠΑ'; ----> Problem_case = 1;
%Problem_case = 1;
%problem_case = 'ΑΝΤΙΡΟΠΑ'; ----> Problem_case = 0;
Problem_case = 0;
...
if Re == 500                   %%%% YOU NEED PATIENCE HERE !!!%%%%
dt = 1.5;           
...
Vort(end,:) = 2*(Psi(end,:)-Psi(end-1,:))/(StepY^2);            %%% BOTTOM WALL
  if Problem_case == 1
    ...
    Vort(1,:) = (2*(Psi(1,:)-Psi(2,:))/(StepY^2)) + (2*U/StepY);    %%% TOP WALL  OPPOSITE  
    end
...
%%% plot vorticity %%%
figure
contour(0:0.0201:1,0:0.0201:1,flip(Vort),[18:-1:-6],'LineWidth', 1 ,'ShowText','on')
title('Reynolds = 1 IMPLICIT, Opposite direction <---')
...
%%% plot Psi %%%
figure
contour(0:0.0201:1,0:0.0201:1,flip(Psi),[-0.01:0.01:0.09],'LineWidth', 1 ,'ShowText','on')
title('Reynolds = 1 IMPLICIT, Opposite direction <---')
xlabel('Πλάτος κοιλώματος [m]')
ylabel('Μήκος κοιλωματος [m]')
...
%%% FOR REYNOLDS = 100 %%%
contour(0:0.0201:1,0:0.0201:1,flip(Vort),[18:-1:-6],'LineWidth', 1 ,'ShowText','on')
title('Reynolds = 100 IMPLICIT, Opposite direction <---')
xlabel('Πλάτος κοιλώματος [m]')
ylabel('Μήκος κοιλωματος [m]')
legend('Στροβιλότητα')
grid on
...
if Re==500
%%% FOR REYNOLDS = 500 %%%
contour(linspace(0,1,Xpoints),linspace(0,1,Xpoints),flip(Vort),[18:-1:-6],'LineWidth', 1 ,'ShowText','on')
title('Reynolds = 500 IMPLICIT, Opposite direction <---')
xlabel('Πλάτος κοιλώματος [m]')
ylabel('Μήκος κοιλωματος [m]')
legend('Στροβιλότητα')
grid on
...
%%% plot Psi %%%
figure
contour(linspace(0,1,Xpoints),linspace(0,1,Xpoints),flip(Psi),[-0.01:0.01:0.09],'LineWidth', 1 ,'ShowText','on')
title('Reynolds = 500 IMPLICIT, Opposite direction <---')
xlabel('Πλάτος κοιλώματος [m]')
ylabel('Μήκος κοιλωματος [m]')
legend('Ροική Συνάρτηση')
